...
test('thunk with dependency injection', (assert) => {
  ...
  assert.ok(action.calledWith({ ...store, config }), 'dependency injection works');
  assert.ok(store.dispatch.calledWith('getConfig'), 'injector can call dispatch');
  assert.ok(store.getState.calledWith('getConfig'), 'injector can call getState');
